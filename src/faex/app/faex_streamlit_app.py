...
def is_classification_dataset(dataset_key: str) -> bool:
    """Heuristic to determine if a dataset is for classification."""
    return dataset_key in CLASSIFICATION_DATASET_OPTIONS
...
CLASSIFICATION_MODEL_OPTIONS = {
    "Random Forest Classifier 20": RandomForestClassifier(n_estimators=20, random_state=42),
    "Random Forest Classifier 100": RandomForestClassifier(n_estimators=100, random_state=42),
    "Neural Network Classifier 5-5": MLPClassifier(max_iter=1000, hidden_layer_sizes=(5, 5)),
    "Neural Network Classifier 10-10": MLPClassifier(max_iter=1000, hidden_layer_sizes=(10, 10)),
...
def get_app_config() -> Dict[str, Any]:
    ...
    if "max_samples" not in st.session_state:
        st.session_state.max_samples = 300
...
def train_regression_model(model: Any, X: Any, y: Any) -> Any:
...
def train_classification_model(model: Any, X: Any, y: Any) -> Any:
    """Fit model with 5-fold CV and return trained model and scores."""
...
def create_core(dataset_key: str, model_key: str, feature_name: str) -> Any:
    ...
    if is_classification_dataset(dataset_key):
        model, cv_scores = train_classification_model(model_obj, df_x, df_y)
        datacore = ClassificationDataCore(
            model=model,
            df_X=df_x,
            class_label=df_y.unique().tolist()[0],
        ...
        model, cv_scores = train_regression_model(model_obj, df_x, df_y)
        datacore = DataCore(
            model=model,
            df_X=df_x,
            study_features=[feature_name],
    ...
    core.score = cv_scores  # attach score for reference TODO: fix
...
def render_sidebar() -> None:
    ...
    with st.sidebar.expander("Create new tab", expanded=True):
        ...
        # 3. Max samples as
        st.session_state.max_samples = st.number_input(
            "Max samples",
            min_value=0,
        ...
        # Select only models compatible with dataset type
        ...
        if is_classification_dataset(dataset_key):
            ...
            model_key = st.selectbox(
                "Model",
                options=list(REGRESSION_MODEL_OPTIONS.keys()),
                key="new_tab_model",
            )
...
def render_tab_content(tab_id: str, tab_state: TabState) -> None:
    ...
    with center_col:
        ...
        if hasattr(core, "score"):
            st.metric(
                label="CV R² (5-fold) / f1-score",
                value=f"{np.mean(core.score):.3f} ± {np.std(core.score):.3f}",
            )
