...
def get_app_config() -> Dict[str, Any]:
    ...
    if "plot_backend" not in st.session_state:
        st.session_state.plot_backend = "Plotly"  # or "Matplotlib" as default

    if "bins" not in st.session_state:
        st.session_state.bins = 100

    if "sigma_factor" not in st.session_state:
        st.session_state.sigma_factor = 1.0

    return {
        "plot_backend": st.session_state.plot_backend,
        "bins": st.session_state.bins,
        "sigma_factor": st.session_state.sigma_factor,
    }
...
def render_sidebar() -> None:
    ...
    with st.sidebar.expander("Create new tab", expanded=True):
        ...
        # 3. Sigma factor
        st.session_state.sigma_factor = st.number_input(
            "Sigma factor",
            min_value=0.0,
            value=cfg["sigma_factor"],
            step=0.1,
        )
        ...
        default_name = f"{dataset_key} {st.session_state.next_tab_id}"
        tab_name = st.text_input("Tab name", value=default_name, key="new_tab_name")
